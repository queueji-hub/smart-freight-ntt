"""QUOTATION VIEW - ONLY Quotation Management.

This file MUST NOT contain any Shipment Console or Dashboard form.
NO Smart Freight Console here.
"""
...
st.caption("จัดการใบเสนอราคา - สร้าง / แก้ไข / คัดลอก / ดาวน์โหลด PDF")
...
def _items_editor(prefix, d):
    ...
    h[4].markdown("**Remark**")
    h[5].markdown("**⬆**"); h[6].markdown("**⬇**")
    h[7].markdown("**⤴**"); h[8].markdown("**⤵**"); h[9].markdown("**🗑**")
    for i in range(len(items)):
        ...
        if c[5].button("⬆", key=f"{prefix}_up_{i}", disabled=(i==0), help="เลื่อนขึ้น"):
            items[i-1], items[i] = items[i], items[i-1]; st.rerun()
        if c[6].button("⬇", key=f"{prefix}_dn_{i}", disabled=(i==len(items)-1), help="เลื่อนลง"):
            items[i+1], items[i] = items[i], items[i+1]; st.rerun()
        ...
        if c[8].button("⤵", key=f"{prefix}_insdn_{i}", help="แทรกข้างล่าง"):
            ...
            st.rerun()
        if c[9].button("🗑", key=f"{prefix}_del_{i}", disabled=(len(items)<=1), help="ลบ"):
