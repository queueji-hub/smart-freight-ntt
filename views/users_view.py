...
def render():
    ...
    with tab_list:
        ...
        # ค้นหา User เพื่อแก้ไข
        search_user = st.selectbox("Select user to manage:", [u["username"] for u in users])
        target = next(u for u in users if u["username"] == search_user)
        ...
        with col2:
            ...
            new_role = st.selectbox("Role", list(PERMISSIONS.keys()), index=list(PERMISSIONS.keys()).index(target["role"]))
