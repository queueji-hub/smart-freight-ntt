...
These helpers are idempotent and non-destructive. Production migrations remain
the durable schema mechanism; the runtime repairs prevent legacy preview
instances from crashing before their migration set has been applied.
"""
...
def ensure_phase30_bl_schema(conn) -> None:
    """Create/upgrade the production-facing B/L header table safely."""
    with conn.cursor() as cur:
        cur.execute(
            """
