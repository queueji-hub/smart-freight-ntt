"""Smart Freight NTT — Main Dashboard (entry point).

Streamlit native multi-page: pages/ folder is auto-discovered.
URLs: / (Dashboard), /Quotation, /Shipments
"""
...
with col_form:
    ...
    container_no = st.text_input("Container No.",
        value=d.get("container_no") or "", key="cc_container_no")
        ...
        if st.button(save_label, type="primary", use_container_width=True,
            ...
            if not customer:
                ...
                data = {
                    ...
                    "customer_name": customer, "carrier": carrier,
                    "pol": pol, "pod": pod, "container_size": size,
...
with col_table:
    ...
    with fcol1:
        filter_status = st.selectbox("Filter", ["All"] + status_options,
    ...
    if not filtered:
        ...
        for r in filtered[:50]:
            ...
            status_class = {
                "In-Progress": "s-progress", "Finished": "s-finish",
                "Cancelled": "s-cancel", "SOC": "s-soc",
                "On-Hold": "s-transit",
            }.get(status, "s-draft")
